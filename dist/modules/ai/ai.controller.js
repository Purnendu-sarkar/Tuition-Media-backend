...
async function generateCoverLetter(req, res, next) {
    try {
        ...
        const coverLetter = await aiService.generateCoverLetter(job.title, job.description, user.name, user.tutorProfile?.bio || undefined);
...
async function optimizeBio(req, res, next) {
    try {
        const { bio } = req.body;
        const userId = req.user?.sub;
        if (!userId) {
