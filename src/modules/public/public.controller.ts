...
async function getTutors(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    if (minRate || maxRate) {
      ...
      if (minRate) where.tutorProfile.hourlyRate.gte = Number(minRate);
      if (maxRate) where.tutorProfile.hourlyRate.lte = Number(maxRate);
    }

    if (query) {
      where.OR = [
        { name: { contains: query as string, mode: "insensitive" } },
        { tutorProfile: { bio: { contains: query as string, mode: "insensitive" } } },
      ];
    }
