...
async function getDashboardStats(tutorId: string) {
  ...
  if (user.image) completenessScore += 20; else missingSteps.push("Upload a profile photo");
  if (profile?.bio) completenessScore += 20; else missingSteps.push("Add a professional bio");
  if (profile?.subjects && profile.subjects.length > 0) completenessScore += 20; else missingSteps.push("List your expert subjects");
...
async function updateProfile(tutorId: string, data: any) {
  ...
  if (profileData.bio !== undefined) cleanProfileData.bio = profileData.bio;
  if (profileData.subjects !== undefined) cleanProfileData.subjects = profileData.subjects;
  if (profileData.location !== undefined) cleanProfileData.location = profileData.location;
  if (profileData.hourlyRate !== undefined) cleanProfileData.hourlyRate = Number(profileData.hourlyRate);
